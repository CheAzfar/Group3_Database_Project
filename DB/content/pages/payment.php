...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($deliveryType === 'dine-in' && empty($tableNo)) $errors[] = "Table number is required for dine-in";
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            ");
            $stmt->bind_param("sidsss", $custName, $tableNo, $total, $deliveryType, $contactPhone, $notes);
            $stmt->execute();
            ...
            if ($isBuyNow) {
                ...
                $itemStmt->bind_param("iiidd", $orderId, $itemId, $item['quantity'], $item['price'], $subtotal);
                $itemStmt->execute();
        ...
        <div class="row">
            <div class="col-lg-8 mx-auto">
                <div class="card shadow-sm" style="background-color: #ffe9dd;">
                    <div class="card-header bg-warning text-dark"><h4 class="mb-0">Payment Information</h4></div>
                    <div class="card-body">
                        <?php if (!empty($errors)): ?>
                        ...
                        <form method="POST">
                            <div class="mb-3"><label for="custName" class="form-label">Full Name</label><input type="text" class="form-control" id="custName" name="custName" value="<?= htmlspecialchars($_POST['custName'] ?? '') ?>" required></div>
                            <div class="mb-3"><label for="contactPhone" class="form-label">Contact Phone</label><input type="tel" class="form-control" id="contactPhone" name="contactPhone" value="<?= htmlspecialchars($_POST['contactPhone'] ?? '') ?>" required></div>

                            <div class="mb-3">
                                <label class="form-label">Order Type</label>
                                <div class="btn-group w-100" role="group">
                                    <input type="radio" class="btn-check" name="deliveryType" id="dineInOption" value="dine-in" <?= ($orderType === 'dine-in') ? 'checked' : '' ?>>
                                    <label class="btn btn-outline-warning" for="dineInOption">Dine-In</label>
                                    <input type="radio" class="btn-check" name="deliveryType" id="takeawayOption" value="takeaway" <?= ($orderType === 'takeaway') ? 'checked' : '' ?>>
                            ...
                            <div class="mb-3" id="tableNoGroup" style="<?= ($orderType === 'takeaway') ? 'display: none;' : '' ?>">
                                ...
                                <select class="form-select" id="tableNo" name="tableNo">
                                    ...
                                    <?php for ($i = 1; $i <= 20; $i++): ?>
                                        <option value="<?= $i ?>" <?= (($_POST['tableNo'] ?? '') == $i ? 'selected' : '') ?>>Table <?= $i ?></option>
                                    <?php endfor; ?>
                                </select>
                            </div>

                            <div class="mb-3"><label for="notes" class="form-label">Special Instructions</label><textarea class="form-control" id="notes" name="notes" rows="3"><?= htmlspecialchars($_POST['notes'] ?? '') ?></textarea></div>

                            <div class="card mb-4">
                                <div class="card-header bg-light"><h5 class="mb-0">Order Summary</h5></div>
                                <div class="card-body">
                                    <div class="mb-3"><strong>Order Date:</strong> <?= date('F j, Y - g:i A') ?></div>
                                    <ul class="list-group list-group-flush">
                                        <?php if ($isBuyNow): ?>
                                            <li class="list-group-item d-flex justify-content-between">
                                                <span><?= htmlspecialchars($buyNowItem['name']) ?> × <?= $buyNowItem['quantity'] ?></span>
                                                <span>RM <?= number_format($buyNowItem['price'] * $buyNowItem['quantity'], 2) ?></span>
                                            </li>
                                        <?php else: ?>
                            ...
                            <div class="d-grid gap-2">
                                ...
                                <a href="cancel_buy_now.php" class="btn btn-outline-secondary">Back to Cart</a>
                            </div>
                        </form>
                    </div>
                </div>
